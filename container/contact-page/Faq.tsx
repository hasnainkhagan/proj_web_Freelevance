...
export default function Faq() {
  ...
  return (
    <section className="w-full padding-y mt-[-10px] bg-background z-30 relative rounded-t-[20px]">
      <h1 className="sub-heading padding-x font-medium font-NeueMontreal text-secondary pb-[50px] capitalize">
        A few things you <br />
        may want to ask us :
      </h1>
      {FaqItems.map((item) => (
        ...
        >
          <div className="w-full flex items-center justify-between sm:gap-[15px] xm:gap-[15px] py-[10px] padding-x">
            <div className="w-[50%] sm:w-full xm:w-full">
              <h1 className="paragraph font-normal font-NeueMontreal text-secondary">
                {item.question}
              </h1>
            </div>
            <div className="w-[50%] sm:w-full xm:w-full flex items-center justify-between">
              <div>
                <h3 className="paragraph font-normal font-NeueMontreal text-secondary">
                  {item.title}
                </h3>
              </div>
                ...
                {activeAccordion === item.id && (
                  ...
                  >
                    <div className="flex flex-col gap-[20px] py-[30px]">
                      <div className="">
                        <p className="paragraph tracking-wider font-normal font-NeueMontreal text-secondary">
                          {item.description}
                        </p>
                      </div>
                      {item.links.map((link) => (
                        ...
                        >
                          <span className="paragraph tracking-wider font-normal font-NeueMontreal text-secondary">
                            {link.title}
                          </span>
                          <p className="paragraph tracking-wider font-normal font-NeueMontreal text-secondary">
                            {link.description}
